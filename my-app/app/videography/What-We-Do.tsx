        ...
        <div className="w-full flex flex-row text-white items-end h-[400px] relative mb-20">
          {(<div className={`relative w-1/2 h-full ${user_hero.user && `group`}`}>
            ...
            {user_hero.user && (<form>
              <label
                htmlFor="upload"
                className={`absolute top-1/2 -translate-y-1/2 left-1/2 ${finished && "hover:bg-transparent hover:text-white"} transform -translate-x-1/2 inline-block cursor-pointer px-5 py-3 border text-xl transition-colors duration-300 border-white rounded-md opacity-0 group-hover:opacity-100 hover:cursor-pointer hover:bg-white hover:text-black`}
                role="button"
                tabIndex={0}
              >
                Upload
